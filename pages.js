...
function getPage ( numPage ) {
	...
	if (numPage == test++ ) return [
		'La première fonction en Javascript', 'PrimFunc', false,
	...
	if (numPage == test++ ) return [
		'Fonctions en Javascript', 'FuncsJS', false,
	...
	if (numPage == test++ ) return [
		'Choix de l’ordinateur, choix fixe au départ', 'ChoixOrdinateur', false,
	...
	if (numPage == test++ ) return [
		'Fonction pour le choix de l’ordinateur', 'FuncChoixOrdi', false,
	...
	if (numPage == test++ ) return [
		'Modifier la fonction choixAleatoire()', 'MofidFuncChoixAle', false,
	...
	if (numPage == test++ ) return [
		'Distinguer entre caillou et ciseaux', 'DistinguerCC', false,
	...
	if (numPage == test++ ) return [
		'Afficher le résultat: qui a gagné, dabord identifier l\'égalité', 'VerifierEgalite', false,
	...
	if (numPage == test++ ) return [
		'Afficher le résultat: qui a gagné, moi ou l\'ordinteur', 'Moi ou Ordinateur', false,
	...
	if (numPage == test++ ) return [
		'Afficher notre choix comme image au lieu de texte', 'ImgNotreChoix', false,
	...
	if (numPage == test++ ) return [
		'Montrer le choix de l’ordinateur aussi comme image', 'ImgChoixOrdi', false,
	...
	if (numPage == test++ ) return [
		'Changer l’affichage des images', 'ImgsFaceAFace', false,
	...
	if (numPage == test++ ) return [
		'Garder un score des résultats', 'ScoreResult', false,
	...
	if (numPage == test++ ) return [
		'Un peu de CSS', 'CSSButtons', false,
	...
	if (numPage == test++ ) return [
		'Fonction choixTriché()', 'ChoixTriche', false,
	...
	if (numPage == test++ ) return [
		'Option entre un jeu honnête ou triché', 'CheckBox', false,
	...
	if (numPage == test++ ) return [
		'Une carte “Google Maps” dans notre page web', 'EmbedGMaps', false,
	...
	if (numPage == test++ ) return [
		'Plusieurs marqueurs sur la page', 'SeveralMarkers', false,
	...
	if (numPage == test++ ) return [
		'Markers en diagonal', 'Diagonal', false,
	...
	if (numPage == test++ ) return [
		'Markers dans un rectangle', 'Rectangle', false,
